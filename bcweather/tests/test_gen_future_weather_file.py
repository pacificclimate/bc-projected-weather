...
from bcweather import gen_prism_offset_weather_file
...
def test_get_ensemble_averages():
    ...
    # "/storage/data/climate/downscale/BCCAQ2+PRISM/ \
    # high_res_downscaling/bccaq_gcm_bc_subset/"
    ...
    proj_files = []
    for gcm in gcms:
        past_file = glob.glob(gcm_dir + gcm + '/'
                              + cdfvariable + '_day_*'
                              + rcp + '*19500101-21001231.nc')
        proj_file = glob.glob(gcm_dir + gcm + '/'
                              + cdfvariable + '_day_*'
                              + rcp + '*19500101-21001231.nc')
